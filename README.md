...
>c) Read In Expression Data: This step takes in the ballgown data tables 
...
>g) Add gene names and gene IDs to the results_transcripts data frame
...
>h) Sort the results from the smallest P value to the largest and write the top 10 results to a CSV
...
>i) Plot Distribution of transcript count per gene
...
>j) Plot Distribution of transcript lengths
...
>l) Plot Distribution of FPKM for all 4 libraries
...
>m) Plot expression values for a pair of wild-type replicates
...
>o) View the distribution of differential expression values with significance p < 0.1
...
>p) Make plots of individual transcripts across samples
...
>q) Display the grand expression values from UHR and HBR and mark those that are significantly differentially expressed
...
>r) significant transcripts with a log2 fold-change >= 2
...
>s) View selected columns of the first 25 lines of output
